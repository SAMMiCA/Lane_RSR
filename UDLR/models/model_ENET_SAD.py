...
from models.loss import mIoULoss, to_one_hot
...
class ENet_SAD(nn.Module):
    ...
    def at_gen(self, x1, x2):
        ...
        if x1.size() != x2.size():
            x1 = torch.sum(x1 ** 4, dim=1, keepdim=True)
            x1_at = torch.squeeze(x1, dim=1)
            x1_at = sps(x1_at)
            x2 = torch.sum(x2 ** 4, dim=1, keepdim=True)
            x2_at = torch.squeeze(self.at_gen_upsample(x2), dim=1)
            x2_at = sps(x2_at)
        else:
            x1 = torch.sum(x1 ** 4, dim=1, keepdim=True)
            x1_at = torch.squeeze(x1, dim=1)
            x1_at = sps(x1_at)
            x2 = torch.sum(x2 ** 4, dim=1, keepdim=True)
            x2_at = torch.squeeze(x2, dim=1)
            x2_at = sps(x2_at)

        x1 = torch.nn.Tanh()(torch.nn.InstanceNorm2d(1)(x1))
        x2 = torch.nn.Tanh()(torch.nn.InstanceNorm2d(1)(x2))
        loss = self.at_gen_l2_loss(x1_at, x2_at)
        return loss, x1, x2

    def forward(self, img, seg_gt=None, exist_gt=None, sad_loss=False):
        # Stage 0 - Initial block
        input_size = img.size()
        x1_at, x2_at, x3_at, x4_at = None, None, None, None
        ...
        if self.sad:
            loss_2, x1_at, x2_at = self.at_gen(x_1, x_2)
        ...
        if self.sad:
            loss_3, _, x3_at = self.at_gen(x_2, x_3)
        ...
        if self.sad:
            loss_4, _, x4_at = self.at_gen(x_3, x_4)
        ...
        return seg_pred, exist_pred, loss_seg, loss_exist, loss, [x1_at, x2_at, x3_at, x4_at]
